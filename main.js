const API_KEY = "268c97a5fd9eee14d926bb2a3a8d9197";
...
const getHourlyForecast = async ({ name: city }) => {
  ...
  return data.list.map((forecast) => {
    ...
    return { temp, temp_max, temp_min, icon, dt, dt_txt, description };
  });
};
    ...
    ([day, { temp_max, temp_min, icon }], index) => {
      dayWiseInfo += ` 
        <article class="day-wise-forecast">
            ...
            <p class="min-temp">${formatTemperature(temp_max)} </p>
            <p class="max-temp">${formatTemperature(temp_min)}  </p>
          </article>`;
    }
  );
